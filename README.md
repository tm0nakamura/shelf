...
消費コンテンツの足跡を、自動で集めて棚にする日本向け Web サービス。リポジトリ名・パッケージ名は歴史的経緯で `shelf-jp` のままです（タナログ = 棚 + ログ）。
...
### 6. （オプション）Steam Web API

PC ゲーム連携。OpenID なので OAuth クライアント登録は不要、API key だけ取れば動きます。

1. [steamcommunity.com/dev/apikey](https://steamcommunity.com/dev/apikey) にログイン
2. ドメイン入力欄に shelf-jp の本番ドメイン（例：`shelf-wine.vercel.app`）を入れる
3. 利用規約に同意 → API key 発行
4. `.env.local` の `STEAM_API_KEY` に貼る

ユーザー側の準備：Steam → 自分のプロフィール → 「プロフィールの編集」 → プライバシー設定 → **「ゲームの詳細」を「公開」**にしないと所有ゲーム一覧が取れません。"全プレイヤー" でも "フレンドのみ" でも OK ですが、"非公開" だと Steam Web API が空配列を返します。

### 7. （オプション）Spotify Developer App
...
| `/api/steam/connect` | Steam OpenID 開始 |
| `/api/steam/callback` | Steam OpenID 戻り → 初回同期 |
| `/api/steam/sync` (POST) | Steam 手動同期 |
...
| `/settings/unext/connect` | U-NEXT cURL 貼り付け連携フォーム |
| `/api/unext/connect` (POST) | U-NEXT cookie 受け取り → 初回同期 |
| `/api/unext/sync` (POST) | U-NEXT 手動同期 |
