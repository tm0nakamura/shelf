  ...
  return (
    <main className="min-h-dvh bg-[#1a1614] text-white px-6 py-12">
      <div className="max-w-xl mx-auto">
        ...
        {params.ok === 'spotify' && (
          <Banner kind="ok">Spotify を連携しました。</Banner>
        )}
        {params.error && (
        ...
        <ul className="space-y-3">
                ...
                <div className="text-xs text-white/50 mt-0.5">
                  {spotify
                    ? spotify.status === 'active'
                      ? `連携中 · 最終同期 ${formatTime(spotify.last_synced_at)}`
                      : `エラー (${spotify.error_count})`
                    : '再生履歴 + Saved Tracks を取得'}
                </div>
              </div>
              <ConnectionActions connected={!!spotify} />
            </div>
          </li>
          ...
          <li className="rounded-2xl border border-white/10 bg-white/[0.03] p-5 opacity-50">
            <div className="flex items-center gap-4">
              ...
              <div className="flex-1">
                <div className="font-bold">Gmail (Amazon 購入)</div>
                <div className="text-xs text-white/50 mt-0.5">Phase 2 で実装予定</div>
              </div>
            </div>
          </li>
